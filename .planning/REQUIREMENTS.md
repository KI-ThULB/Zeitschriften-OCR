...
- [x] **OPER-02**: `--verbose` flag prints Tesseract stdout/stderr and per-stage wall-clock timing (deskew, crop, OCR, write) for each processed file
- [x] **OPER-03**: During batch processing, a live progress line shows files completed, total count, percentage, and estimated time remaining (updated as each file completes)
- [x] **OPER-04**: `--config PATH` loads CLI flag defaults from a JSON file; any flag specified on the command line overrides the config value
- [x] **OPER-05**: If `--config PATH` is specified but the file does not exist or is not valid JSON, pipeline exits with a clear error message before any processing begins
...
| OPER-04 | Phase 8 | Complete |
| OPER-05 | Phase 8 | Complete |
